...
class _PagesTestWidgetState extends State<PagesTestWidget> with AfterLayoutMixin<PagesTestWidget> {
  initState() {
    super.initState();
    ...
    setState(() {
      ...
      switch (tabItem) {
        case 0:
          ...
          break;
        case 1:
          widget.currentPage = ProviderHome(parentScaffoldKey: widget.scaffoldKey);
          break;
  ...
  void afterFirstLayout(BuildContext context) async {
    ...
    if (currentUser.value.apiToken != null) {
      FirebaseFirestore.instance.collection("orderDetails").where("driverId", isEqualTo: currentUser.value.id).where("status", isEqualTo: 'RShipped').where("driverStatus", isEqualTo: 'Waiting').snapshots().listen((snapshot) {
  ...
  void _showAlert(BuildContext context, course, tab) {
    loadCallMusic();
    showDialog(context: context, barrierDismissible: false, builder: (context) => ExitConfirmationDialogranger(course: course, currentTab: tab, player: player));
  }
...
class ExitConfirmationDialogranger extends StatefulWidget {
  ExitConfirmationDialogranger({Key key, this.course, this.currentTab, this.player}) : super(key: key);
  DocumentSnapshot course;
  dynamic currentTab;
  final player;
  @override
  _ExitConfirmationDialograngerState createState() => _ExitConfirmationDialograngerState();
}

class _ExitConfirmationDialograngerState extends StateMVC<ExitConfirmationDialogranger> {
          ...
          child: Column(
            children: <Widget>[
              ...
              Container(
                ...
                decoration: BoxDecoration(color: Colors.white, shape: BoxShape.rectangle, borderRadius: BorderRadius.only(topLeft: Radius.circular(12), topRight: Radius.circular(12))),
              ),
              SizedBox(
                height: 24,
              ),
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    RawMaterialButton(
                      onPressed: () async {
                        _con.statusAccepted('Accepted', widget.course['orderId'], '');
                        //  widget.currentTab(2);
                        await widget.player.stop();
                        Navigator.of(context).pop(true);
                      },
